...
# Avvia worker TTS all'avvio del server
load_qwen3_model()
        ...
        try:
            if self.path in ("/","/index.html"):
                ...
                self._json(200,{"status":"online","model":cfg["groq"]["model"],
                    "voice":cfg["tts"].get("qwen3_voice", cfg["tts"].get("voice", "vivian")),"version":"8.0",
                ...
                from jarvis_mail import get_unread_count
                self._json(200, {"unread": get_unread_count()})
...
if __name__=="__main__":
    print()
    print("═"*52)
    print("  J.A.R.V.I.S  SERVER  v8.0 — ADVANCED AI ASSISTANT")
    print("═"*52)
    print(f"  URL  →  http://localhost:{PORT}")
    print(f"  LLM  →  {cfg['groq']['model']}")
    print(f"  TTS  →  Qwen3-TTS (MLX) + Streaming")
    ...
    print(f"  Browser → Playwright automation")
